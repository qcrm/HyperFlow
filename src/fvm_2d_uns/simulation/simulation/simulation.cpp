...
void Simulation::solve() {
    ...
    for (unsigned int n=1; n<Simulation::MAX_STEPS; n++) {
        ...
        auto evolve_func = std::bind(&Scheme::operator(), scheme);
        Vec2D cell_update = ode_solver->operator()(evolve_func, mesh->get_cells_flow_values(), dt);
        mesh->set_cells_flow_values(cell_update);
